/* ================================================
   HEMATKU — app.js
   Logika aplikasi & penyimpanan LocalStorage
   ================================================ */
...
var TIPS = [
  ...
  '🧋 Kurangi minuman kekinian — 1 minuman Rp20.000 x 5x seminggu = Rp100.000!',
...
/* ── Ikon & Warna Kategori ── */
var ICON = {
  Makan: '🍛', Transport: '🚌', Jajan: '🍭', Belajar: '📚', Lainnya: '📦'
};
var ICCLS = {
  Makan: 'ic-makan', Transport: 'ic-transport', Jajan: 'ic-jajan',
  Belajar: 'ic-belajar', Lainnya: 'ic-lainnya'
};
var BARCOL = {
  Makan: '#f59e0b', Transport: '#3b82f6', Jajan: '#ec4899',
  Belajar: '#22c55e', Lainnya: '#8b5cf6'
};
...
function doLogin() {
  ...
  if (!email || !pass) {
    toast('Peringatan', 'Email dan password harus diisi!', 'warn'); return;
  }
  if (email.indexOf('@') === -1) {
    toast('Format Salah', 'Masukkan email yang valid!', 'error'); return;
  }
  ...
  if (!found) {
    toast('Gagal Masuk', 'Email atau password salah!', 'error'); return;
  }

  user = found;
  localStorage.setItem('hku_user', JSON.stringify(user));
  toast('Berhasil!', 'Selamat datang, ' + user.name + '!', 'success');
  initApp();
}
...
function doRegister() {
  ...
  if (!name || !email || !pass) {
    toast('Peringatan', 'Semua kolom harus diisi!', 'warn'); return;
  }
  if (email.indexOf('@') === -1) {
    toast('Format Salah', 'Masukkan email yang valid!', 'error'); return;
  }
  if (pass.length < 6) {
    toast('Password Lemah', 'Password minimal 6 karakter!', 'warn'); return;
  }

  var users = JSON.parse(localStorage.getItem('hku_users') || '[]');
  for (var i = 0; i < users.length; i++) {
    if (users[i].email === email) {
      toast('Email Dipakai', 'Email sudah terdaftar!', 'error'); return;
    }
  }
  ...
  toast('Akun Dibuat!', 'Halo ' + name + ', selamat datang di HematKu!', 'success');
  initApp();
}
...
function initApp() {
  ...
  var nama  = user.name.split(' ')[0];
  document.getElementById('greeting').textContent = salam + ', ' + nama + '!';
...
function getData() {
  return JSON.parse(localStorage.getItem(key('trx')) || '[]');
}

function saveData(d) {
  localStorage.setItem(key('trx'), JSON.stringify(d));
}

function getBatas() {
  return parseInt(localStorage.getItem(key('batas')) || '300000');
}
...
function simpan() {
  ...
  if (!tgl) {
    toast('Peringatan', 'Pilih tanggal dulu!', 'warn'); return;
  }
  if (!jml || jml <= 0) {
    toast('Peringatan', 'Masukkan jumlah yang valid!', 'warn'); return;
  }
  ...
  toast('Tersimpan!', 'Rp ' + jml.toLocaleString('id-ID') + ' - ' + kat + ' berhasil dicatat.', 'success');
  cekBatas(data);
  renderAll();
}
...
function hapusSemua() {
  if (getData().length === 0) {
    toast('Kosong', 'Tidak ada data untuk dihapus.', 'info'); return;
  }
...
function simpanBatas() {
  ...
  if (!v || v <= 0) {
    toast('Peringatan', 'Masukkan batas yang valid!', 'warn'); return;
  }
...
function renderBeranda() {
  ...
  for (var j = 0; j < data.length; j++) {
    if (data[j].tanggal.indexOf(mo) === 0) {
      kt[data[j].kategori] = (kt[data[j].kategori] || 0) + data[j].jumlah;
    }
  }
  ...
  if (!katKeys.length) {
    katEl.innerHTML = '<p style="font-size:13px;color:var(--txt3);text-align:center;padding:16px 0">Belum ada pengeluaran bulan ini.</p>';
  } else {
    var mx = 0;
    ...
    for (var ki = 0; ki < katKeys.length; ki++) {
      ...
      html +=
        ...
        '<div class="kat-bar-hd">' +
        '<span class="kat-bar-lbl">' + (ICON[k2] || '📦') + ' ' + k2 + '</span>' +
        '<span class="kat-bar-val">Rp ' + kt[k2].toLocaleString('id-ID') + '</span>' +
        '</div>' +
        '<div class="kat-bar-track">' +
        '<div class="kat-bar-fill" style="width:' + pctK + '%;background:' + (BARCOL[k2] || '#8b5cf6') + '"></div>' +
        '</div></div>';
    }
  ...
  if (!data.length) {
    ...
    for (var ri = 0; ri < max; ri++) recentHtml += trxHTML(data[ri], false);
...
function renderRiwayat() {
  ...
  if (!data.length) {
    ...
    var html = '';
    for (var i = 0; i < data.length; i++) html += trxHTML(data[i], true);
    el.innerHTML = html;
  }
}
...
function trxHTML(t, del) {
  ...
  var delBtn = del
    ? '<button class="trx-del" type="button" onclick="hapusTrx(' + t.id + ')">🗑️</button>'
    : '';
  return (
    '<div class="trx-item">' +
...
function cekBatas(data) {
  ...
  if (pct >= 100) {
    toast('⚠️ Batas Terlampaui!',
      'Pengeluaran bulan ini melebihi batas Rp ' + batas.toLocaleString('id-ID') + '!',
      'error', 6000);
  } else if (pct >= 80) {
    toast('Hampir Habis!',
      'Sudah ' + Math.round(pct) + '% dari batas pengeluaran bulan ini!',
      'warn', 5000);
  }
}
...
════════════════════════════════ */
function loadDark() {
  dark = localStorage.getItem('hku_dark') === 'true';
  applyDark();
}

function toggleDark() {
  dark = !dark;
  localStorage.setItem('hku_dark', dark);
  applyDark();
}
